...
const char* SERVER_URL = "http://10.139.175.177:5000/ingest";
// ---------------------------------------------------------------------------------------
...
// ========== State management - must persist across deep sleep ==========
...
void setup() {
  ...
  if (!mpu.begin()) {
    ...
    buzzPattern(5, 100, 100); // Error pattern
    while (1) delay(100);
  }
...
void postBatch() {
  ...
  if (code == 200) {
    ...
    if (resp.indexOf("\"command\":\"JUMP\"") > 0 || 
        resp.indexOf("\"sleep\":true") > 0) {
      http.end();
      // Triple beep alert
      buzzPattern(3, 200, 200);
      enterDeepSleep();  // Go to sleep immediately!
